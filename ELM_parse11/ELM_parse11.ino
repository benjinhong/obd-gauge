...
void setup() {
  ...
  //Serial.println("ATZ"); //set protocol 6
  delay(100);
  Serial.println("ATSP6"); //set protocol 6
  delay(100);
  Serial.println("ATAL"); //allow >7 byte msg
  delay(100);
  Serial.println("ATE0"); //turn off echo
  delay(100);
  Serial.println("ATS0"); //turn off spaces
  delay(100);
...
void loop() {
  ...
  if (1) { //ERR == 0
    ...
    {
      debug.println("POSITIVE PRESSURE");
      valScaled = valScaledPre;
    }*/
...
int getData() { //whatever that is in the character array is still represented in hex.
  ...
  else if (PID_SEL == 2) 
  {
    ...
    Serial.println("010B041"); //MAP ONLY is 010B1. currently polling MAP + LOAD //NORMAL REQUEST
  ...
  */
  ...
  {
    incoming = Serial.read(); //read in character (as integer) and destroy
  ...
  if (response[0] == 'C') { //CAN ERROR
   ...
   lastFlag1 = true; //reset flag for serial debugging in STOPPED mode.
    ...
    /*if (PID_SEL == 1) { //ABP
      debug.println("Setting ABP to LOAD (ABP requested)");
      ABP = LOAD;
      PID_SEL = 0;
    }*/
